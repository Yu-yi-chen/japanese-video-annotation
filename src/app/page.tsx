    ...
    <div className="flex flex-col h-[100dvh] bg-[#0a0d14] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#0a0d14] relative">
          ...
          {/* Transcript Toolbar */}
          {session && (
            <div className="shrink-0 flex items-center justify-between px-4 py-2 border-b border-slate-800/60 bg-slate-900/30">
              <span className="text-xs text-slate-500 font-medium">{session.segments.length} 句</span>
              <div className="flex items-center gap-1">
                {/* Save status */}
                {mounted && saveStatus !== 'idle' && (
                  <div className={clsx('flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium', saveStatus === 'saving' ? 'text-slate-400' : 'text-emerald-400')}>
                    ...
                    {showClearConfirm && (
                      <div className="absolute right-0 top-full mt-2 z-30 p-3 rounded-xl bg-slate-900 border border-slate-700 shadow-2xl min-w-[180px]">
                        ...
                        <div className="flex gap-2">
                          <button onClick={handleClearAll} className="flex-1 py-1.5 text-xs rounded-lg bg-red-700 hover:bg-red-600 text-white font-medium transition-all">清除</button>
                          <button onClick={() => setShowClearConfirm(false)} className="flex-1 py-1.5 text-xs rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-300 transition-all">取消</button>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div ref={transcriptScrollRef} className="flex-1 overflow-y-auto px-4 py-4 transcript-scroll">
